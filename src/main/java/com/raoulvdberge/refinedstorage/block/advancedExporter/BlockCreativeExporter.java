...
public class BlockCreativeExporter extends BlockCable {
    public BlockCreativeExporter() {
        super(createBuilder("creativeexporter").tileEntity(TileCreativeExporter::new).create());
    }
    ...
    public boolean onBlockActivated(@Nonnull World world, @Nonnull BlockPos pos, @Nonnull IBlockState state, @Nonnull EntityPlayer player, @Nonnull EnumHand hand, @Nonnull EnumFacing side, float hitX, float hitY, float hitZ) {
        ...
        return openNetworkGui(RSGui.CREATIVE_EXPORTER, player, world, pos, side);
    }
}
