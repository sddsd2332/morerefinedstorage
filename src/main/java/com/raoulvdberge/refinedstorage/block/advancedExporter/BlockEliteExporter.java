...
public class BlockEliteExporter extends BlockCable {
    public BlockEliteExporter() {
        super(createBuilder("eliteexporter").tileEntity(TileEliteExporter::new).create());
    }
    ...
    public boolean onBlockActivated(@Nonnull World world, @Nonnull BlockPos pos, @Nonnull IBlockState state, @Nonnull EntityPlayer player, @Nonnull EnumHand hand, @Nonnull EnumFacing side, float hitX, float hitY, float hitZ) {
        ...
        return openNetworkGui(RSGui.ELITE_EXPORTER, player, world, pos, side);
    }
}
