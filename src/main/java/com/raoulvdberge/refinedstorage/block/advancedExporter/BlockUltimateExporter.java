...
public class BlockUltimateExporter extends BlockCable {
    public BlockUltimateExporter() {
        super(createBuilder("ultimateexporter").tileEntity(TileUltimateExporter::new).create());
    }
    ...
    public boolean onBlockActivated(@Nonnull World world, @Nonnull BlockPos pos, @Nonnull IBlockState state, @Nonnull EntityPlayer player, @Nonnull EnumHand hand, @Nonnull EnumFacing side, float hitX, float hitY, float hitZ) {
        ...
        return openNetworkGui(RSGui.ULTIMATE_EXPORTER, player, world, pos, side);
    }
}
