...
public class ContainerCreativeImporter extends ContainerBase {
    public ContainerCreativeImporter(TileCreativeImporter importer, EntityPlayer player) {
        ...
        for (int i = 54; i < 63; ++i) {
            addSlotToContainer(new SlotFilter(importer.getNode().getConfig().getItemHandler(), i, 8 + (18 * (i - 9 * 6)), 20 + 18 * 6).setEnableHandler(() -> importer.getNode().getConfig().isFilterTypeItem()));
        }
        for (int i = 63; i < 72; ++i) {
            addSlotToContainer(new SlotFilter(importer.getNode().getConfig().getItemHandler(), i, 8 + (18 * (i - 9 * 7)), 20 + 18 * 7).setEnableHandler(() -> importer.getNode().getConfig().isFilterTypeItem()));
        }
        ...
        for (int i = 54; i < 63; ++i) {
            addSlotToContainer(new SlotFilterFluid(importer.getNode().getConfig().getFluidHandler(), i, 8 + (18 * (i - 9 * 6)), 20 + 18 * 6).setEnableHandler(() -> importer.getNode().getConfig().isFilterTypeFluid()));
        }
        for (int i = 63; i < 72; ++i) {
            addSlotToContainer(new SlotFilterFluid(importer.getNode().getConfig().getFluidHandler(), i, 8 + (18 * (i - 9 * 7)), 20 + 18 * 7).setEnableHandler(() -> importer.getNode().getConfig().isFilterTypeFluid()));
        }
