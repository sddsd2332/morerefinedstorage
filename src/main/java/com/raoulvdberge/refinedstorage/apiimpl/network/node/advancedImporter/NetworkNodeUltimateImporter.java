...
public class NetworkNodeUltimateImporter extends NetworkNode implements IRSFilterConfigProvider, ICoverable, IUpgradeContainer {
    public static final String ID = "ultimateimporter";
    ...
    private final FilterConfig config = new FilterConfig.Builder(this)
            ...
            .filterSizeNine4()
            .compareDamageAndNbt()
            .customFilterTypeSupplier((ft) -> world.isRemote ? FilterType.values()[TileUltimateImporter.TYPE.getValue()] : ft).build();

    private final CoverManager coverManager = new CoverManager(this);

    private int currentSlot;

    public NetworkNodeUltimateImporter(World world, BlockPos pos) {
        super(world, pos);
    }
