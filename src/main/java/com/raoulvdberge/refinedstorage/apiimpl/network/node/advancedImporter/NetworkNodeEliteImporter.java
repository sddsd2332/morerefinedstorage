...
public class NetworkNodeEliteImporter extends NetworkNode implements IRSFilterConfigProvider, ICoverable, IUpgradeContainer {
    public static final String ID = "eliteimporter";
    ...
    private final FilterConfig config = new FilterConfig.Builder(this)
            ...
            .filterSizeNine3()
            .compareDamageAndNbt()
            .customFilterTypeSupplier((ft) -> world.isRemote ? FilterType.values()[TileEliteImporter.TYPE.getValue()] : ft).build();

    private final CoverManager coverManager = new CoverManager(this);

    private int currentSlot;

    public NetworkNodeEliteImporter(World world, BlockPos pos) {
        super(world, pos);
    }
