package com.raoulvdberge.refinedstorage.apiimpl.network.node.advancedImporter;
...
public class NetworkNodeCreativeImporter extends NetworkNode implements IRSFilterConfigProvider, ICoverable, IUpgradeContainer {
    public static final String ID = "creativeimporter";

    private static final String NBT_COVERS = "Covers";

    private final ItemHandlerUpgrade upgrades = new ItemHandlerUpgrade(4, new ListenerNetworkNode(this),
            ItemUpgrade.TYPE_SPEED, ItemUpgrade.TYPE_STACK, ItemUpgrade.TYPE_EXTREME_STACK);
    ...
    private final FilterConfig config = new FilterConfig.Builder(this)
            ...
            .filterSizeNine5()
            .compareDamageAndNbt()
            .customFilterTypeSupplier((ft) -> world.isRemote ? FilterType.values()[TileCreativeImporter.TYPE.getValue()] : ft).build();

    private final CoverManager coverManager = new CoverManager(this);

    private int currentSlot;

    public NetworkNodeCreativeImporter(World world, BlockPos pos) {
        super(world, pos);
    }
